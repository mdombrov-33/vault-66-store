...
export const getEnduranceText = (value: number) => {
  if (value >= 9) return 'You could survive a nuclear blast and still walk away.'
  if (value >= 6) return 'You can take a beating and keep on going.'
  if (value >= 3) return 'You might need a Stimpak or two to get through the day.'
...
export const getLuckText = (value: number) => {
  if (value >= 9) return 'You could find a Nuka-Cola Quantum in a pile of junk.'
  if (value >= 6) return 'You have a knack for finding useful items in the Wasteland.'
  if (value >= 3) return 'You might find a bottlecap or two, but not much else.'
...
export const getCombinedSpecialText = (special: SpecialRecord) => {
  const { strength, perception, endurance, charisma, intelligence, agility, luck } = special
  ...
  if (charisma <= 3 && perception >= 6 && intelligence >= 5 && intelligence <= 7) {
  ...
  if (charisma >= 7 && intelligence >= 5 && intelligence <= 7 && strength <= 3) {
