...
export interface MemoryDumpGridProps extends HackingGameBaseProps, HackingHoverProps {
  leftColumn: string[]
  rightColumn: string[]
}

export interface TerminalIntroProps extends Pick<HackingHoverProps, 'setOnWordHover'> {
  attemptsLeft: number
  resetGame: () => void
  setIsIntroDone: React.Dispatch<React.SetStateAction<boolean>>
}

export type ResetBtnProps = Pick<TerminalIntroProps, 'resetGame' | 'setOnWordHover'>
...
export interface TerminalLogProps extends Pick<HackingGameBaseProps, 'log' | 'gameOver'> {
  onWordHover: string | null
}
