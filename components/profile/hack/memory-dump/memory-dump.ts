...
export function generateLines() {
  ...
  for (let i = 0; i < 16; i++) {
    const leftAddr = '0xF4' + (160 + i * 0x10).toString(16).toUpperCase().padStart(2, '0') + ': '
    const rightAddr = '0xF4' + (168 + i * 0x10).toString(16).toUpperCase().padStart(2, '0') + ': '

    const leftWord = GUESSING_WORDS[Math.floor(Math.random() * GUESSING_WORDS.length)]
    const rightWord = GUESSING_WORDS[Math.floor(Math.random() * GUESSING_WORDS.length)]
