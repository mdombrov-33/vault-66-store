...
function GoatFinalResults({ finalSkills, taggedSkills }: GoatFinalResultsProps) {
  return (
    <div className="max-w-4xl mx-auto text-xl md:-mt-10 px-4 pb-24 md:pb-4">
      <h2 className="text-4xl font-semibold mb-6  uppercase tracking-wide">Skills</h2>
        ...
        {Object.entries(finalSkills).map(([skill, value]) => {
          ...
          return (
            <div
              ...
              role="listitem"
              tabIndex={0}
              aria-label={`${skill} skill, value ${value}, ${isTagged ? 'tagged' : 'not tagged'}`}
            >
              ...
              {isTagged && (
                <div className="text-primary text-2xl select-none" aria-hidden="true">
                  <GiBottleCap />
                </div>
              )}
            </div>
